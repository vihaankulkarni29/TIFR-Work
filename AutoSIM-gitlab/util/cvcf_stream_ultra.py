...
help_txt='''
Use: python3 cvcf_stream_ultra.py <config.txt>
...
python3 cvcf_stream_ultra.py <help/-h/-help/--help>
...
try:
    ...
    # Use the ultra-fast algorithm for CVCF computations
    ...
    if config["verbose"]:
        print("Computing CVCF in a memory-safe ultra-stream")
    
    # Using RAM buffering: accumulate strings without blocking disk I/O per frame
    output_buffer = []
    ...
    for ts in u_traj.trajectory:
        ...
        # Stripped 'Norm' redundancy - direct math ops!
        s1 += coords
        s2 += np.square(coords)
        
        # Calculate cvcf_val directly using raw algebraic sums
        cvcf_val = np.sum((s2 / frames) - np.square(s1 / frames))
        
        # Buffer string mapping dynamically
        output_buffer.append(f"{(frames - 1):10.0f}{cvcf_val:20.5f}\n")
        
        if config["verbose"]:
            print("CVCF Progress: ", (100 * frames / framelen), "%", end="\r")

    # Flush RAM buffer entirely in one operation at the very end
    with open(os.path.join(cwd, config["outfile"]), "w") as cvcf_out:
        cvcf_out.writelines(output_buffer)

    if config["verbose"]:
        print()
        print("CVCF computed and bulk-flushed to disk successfully")
